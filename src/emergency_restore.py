"""
Emergency Restore Tool for Exam Shield Premium
Run as administrator if Exam Shield crashed mid-lockdown.
"""
...
class EmergencyRestoreApp:
    ...
    def setup_ui(self):
        ...
        tk.Frame(advisory_frame, bg=self.colors['accent'], height=3).pack(fill=tk.X)
        info_text = (
        ...
        self.restore_btn = tk.Button(btn_container, text="🚀 INITIATE SYSTEM REPAIR",
                                     ...
                                     command=self.start_restore, padx=20, pady=12,
                                     activebackground='#c0392b', activeforeground=self.colors['white'])
        ...
        theme_combo = ttk.Combobox(theme_frame, textvariable=self.theme_var,
                                   values=["light", "dark", "pink"], state="readonly",
    ...
    def run_repair_sequence(self):
            ...
            if platform.system().lower() == "windows":
                for adapt in ['Local Area Connection', 'Wi-Fi', 'Ethernet']:
                    subprocess.run(['netsh', 'interface', 'ip', 'set', 'dns',
                                    f'name="{adapt}"', 'source=dhcp'], capture_output=True)
        ...
        messagebox.showinfo("Success",
                            ...
                            "Internet and OS functions should now be unlocked.")
